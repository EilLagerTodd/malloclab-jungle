...
#define LISTLIMIT \
  20  // list의 limit 값을 설정해준다. CHUNKSIZE에 비해 충분히 큰 값을 준 것
      // 같다(정확한 이유 모르겠음)
...
#define GET_SIZE(p) \
  (GET(p) & ~0x7)  // ~0x00000111 -> 0x11111000(얘와 and연산하면 size나옴)
#define GET_ALLOC(p) (GET(p) & 0x1)  // 할당이면 1, 가용이면 0

// Given block ptr bp, compute address of its header and footer
#define HDRP(bp) ((char *)(bp)-WSIZE)
#define FTRP(bp) \
  ((char *)(bp) + GET_SIZE(HDRP(bp)) - DSIZE)  //헤더+데이터+풋터 -(헤더+풋터)
...
static void *coalesce(void *bp) {
  // coalesce는 succ, pred를 보는 것이 아니라 인접한 prev, next 블록을 보는 것을
  // 주의!!!!!!!!!! 전 블록 가용한지
  ...
  if (prev_alloc && next_alloc) {  // 앞,뒤 모두 allocated
    ...
    return bp;
             ...
             !next_alloc)  // 앞은 allocated, 바로 뒤에 free block이 존재할 때
  {
    remove_block(
        NEXT_BLKP(bp));  // 뒤 블록을 일단 지워(나중에 합쳐서 insert 해줄거야)
  ...
  {
    remove_block(
        PREV_BLKP(bp));  // 앞 블록을 일단 지워(나중에 합체하고 insert 해줄거야)
  ...
  insert_block(
      bp, size);  // 드디어 insert! (coalesce를 하면 무조건 insert가 실행됨)
  return bp;
}
...
static void place(void *bp, size_t asize) {
  ...
  remove_block(bp);
  // 필요한 블록 이외에 남는게 16바이트 이상이면 - (header,pred,succ,footer 각각
  // 16byte필요)
...
static void *find_fit(size_t asize) {
  ...
  while (list < LISTLIMIT) {
    ...
    // 나보다 큰 사이즈의 segregation_list가 NULL이 아니면 (나보다 큰 사이즈의
    // list 안에 free 블록이 존재할 경우)
    if ((list == LISTLIMIT - 1) ||
        (searchsize <= 1) && (segregation_list[list] != NULL)) {
...
static void remove_block(void *bp) {
  ...
  if (SUCC_FREE(bp) != NULL) {  // succ 블록이 NULL이 아니면
    if (PRED_FREE(bp) !=
        NULL) {  // pred 블록이 NULL이 아니면 (중간에 있는걸 지우는 경우)
...
static void insert_block(void *bp, size_t size) {
  ...
  void *search_ptr;
  void *insert_ptr =
      NULL;  // search_ptr의 값을 저장해놓는 용도(insert_ptr의 부모같음)

  while ((list < LISTLIMIT - 1) &&
         (size > 1)) {  // segregation_list의 idx를 찾는 과정
    size >>= 1;
    list++;
  }

  search_ptr = segregation_list[list];
  //오름차순으로 저장하기 위해 나보다 작은 놈들은 넘기고 나보다 큰놈 앞에서
  //멈추게 됨
  ...
  if (search_ptr != NULL) {        // search_ptr이 NULL이 아닐 때
    ...
    } else {  // insert_ptr이 NULL일 때 (안에 들어왔는데 내가 제일 작아서 list에
              // 바로 삽입할 때)
    ...
    if (insert_ptr != NULL) {  // 처음 시작할 때는 이 코드가 돌아갈 일이 없지만
      SUCC_FREE(bp) = NULL;  // 진행하다보면 연결 list안에서 내가 제일 커서
                             // search_ptr은 null,
      PRED_FREE(bp) =
          insert_ptr;  // insert_ptr은 현재 list에서 가장 큰 경우가 존재한다.
      SUCC_FREE(insert_ptr) = bp;
