...
#define INITCHUNKSIZE \
  (1 << 6)  // explicit 초기 힙 크기가 6워드짜리 메모리 였으므로.
...
//포인터 p가 가르키는 곳의 값에서 하위 3비트를 제거하여 블럭 사이즈를
//반환(헤더+푸터+페이로드+패딩)
...
//블럭포인터 -> 블럭포인터 - WSIZE : 헤더위치 -> GET_SIZE으로 현재
//블럭사이즈계산 -> 다음 블럭포인터 반환
#define NEXT_BLKP(bp) ((char *)(bp) + GET_SIZE(((char *)(bp)-WSIZE)))
//블럭포인터 -> 블럭포인터 - DSIZE : 이전 블럭 푸터 -> GET_SIZE으로 이전
//블럭사이즈계산 -> 이전 블럭포인터 반환
#define PREV_BLKP(bp) ((char *)(bp)-GET_SIZE(((char *)(bp)-DSIZE)))

//포인터 p가 가르키는 메모리에 포인터 ptr을 입력
#define SET_PTR(p, ptr)   \
  (*(unsigned int *)(p) = \
       (unsigned int)(ptr))  // SET_PTR(주소값, value값) p의 주소값을 블럭에
                             // ptr로 담는다
...
//전역변수
char *heap_listp = 0;
void *segregated_free_lists[LISTLIMIT];  // 크기별로. 20까지 하는 이유? 코드 짠
                                         // 사람이 임의로 설정한 값. 아마
                                         // 최적화이지 않을까.
...
int mm_init(void) {
  ...
  for (list = 0; list < LISTLIMIT;
       list++) {  // 첫번째만 모은 리스트 20요소의 값은 전부 NULL로 채운다
...
static void insert_node(void *ptr, size_t size) {
  int list = 0;
  void *search_ptr = ptr;  // 이진탐색트리의 parent 역할, search_ptr부터 크기가
                           // 맞는 리스트의 영역으로 들어감
  void *insert_ptr =
      NULL;  //실제 노드가 삽입되는 위치, 이진탐색트리의 child 역할

  // Select segregated list
  while ((list < LISTLIMIT - 1) &&
         (size > 1)) {  // 이진수의 3자리 비트 연산, 반복문 한바퀴 돌 때마다
                        // 자릿수 하나씩 쉬프트.
  ...
  search_ptr =
      segregated_free_lists[list];  // 우리가 넣을 사이즈의 seg의 인덱스를 찾음
...
void *mm_malloc(size_t size) {
  ...
  /* 최소 16바이트 크기의 블록 구성. 8바이트 for 정렬 조건 만족 위해, 추가
   * 8바이트 for 헤더, 풋터 오버헤드를 위해 */
  if (size <= DSIZE) {
    asize = 2 * DSIZE;  // adjust size : 조정된 사이즈. 2*8 = 16 , 8보다 작은
                        // 요청값은 다 16으로 만든다
  } else {  // 8바이트 넘는 요청 : 오버헤드 바이트 내에 더해주고, 인접 8의
            // 배수로 반올림
    asize = DSIZE * ((size + (DSIZE) + (DSIZE - 1)) / DSIZE);
  }

  /* Search the free lish for a fit */  // find_fit으로 적절한 가용블록을
                                        // 가용리스트에서 검색
  if ((bp = find_fit(asize)) != NULL) {
    // 맞는 블록 찾으면 - 할당기 : place로 요청 블록 배치, 옵션으로 초과부분
    // 분할, 새 할당 블록 리턴(반환)
    bp =
  ...
  // 못찾으면 : extend_heap으로 힙을 새 가용 블록으로 확장(메모리 공간 더 할당),
  // 요청 블록을 이 새 가용 블록에 배치
...
// 전,후에 free block이 있을시 합쳐줌 + 합쳐지는 경우 segregation_lists에서 기존
// free block 노드 삭제해줌
static void *coalesce(void *bp) {
  size_t prev_alloc =
  ...
  /* case 2 */  // 이전 블럭 최하위 bit 1(할당)이고, 다음 블럭 최하위 bit
                // 0(비할당)이면
  ...
  /* case 3 */  // 이전 블럭 최하위 bit 0(비할당), 다음 블럭 최하위 bit
                // 1(할당)이면
  else if (!prev_alloc && next_alloc) {
    delete_node(bp);
...
static void *find_fit(size_t asize) {
  ...
  while (list < LISTLIMIT) {  // searchsize를 seg_list의 몇번째에 넣을지 찾음
    ...
    /* 	(=) searchsize = searchsize >> 1
        ...
        ex) 4 >> 1 : 100 -> 10, 10 -> 1 두번 쉬프트… i = 2 되고 윗줄이랑 똑같이
       적용 +ex) 3 << 1 : 11 => 110 */
  }
...
static void *place(void *bp, size_t asize) {
  ...
  if (remainder <=
      2 * DSIZE) {  // 남은 사이즈가 16보다 작으면 바로 할당으로 넣기
    PUT(HDRP(bp), PACK(csize, 1));
    PUT(FTRP(bp), PACK(csize, 1));
  } else if (asize >=
             100) {  // test case기준으로 맞춤..ㅎㅎ 100 기준으로 100보다 작거나
                     // 큰 사이즈 몰아서 넣고 free 합침(더 효율적)
    // 요청사이즈가 100보다 크면
    ...
    PUT(HDRP(NEXT_BLKP(bp)),
        PACK(remainder, 0));  // 남은 사이즈만큼 넥스트가 프리 됨
